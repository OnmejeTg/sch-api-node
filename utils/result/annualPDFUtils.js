...
function drawAnnualHeader(doc, pageWidth) {
  ...
  doc.setFontSize(11);
  doc.setFont("helvetica", "bold");
  doc.text("STUDENT'S ACADEMIC REPORT (ANNUAL)", pageWidth / 2, 32, {
    align: "center",
  });
...
function drawAnnualStudentInfo(doc, student, teacher) {
  ...
  doc.text(`Admission ID: ${student.data.studentId.studentId || ""}`, 20, 59);
  doc.text(
  ...
  doc.text(`Class: ${student.data.classLevel.name || ""}`, 160, 54);
  // doc.text(`Total Days Present: ${totalDaysPresent || ""}`, 160, 59);

  doc.line(10, 63, 200, 63);
}

function drawAnnualScoreSheet(doc, result) {
  const { SCORE_SHEET_START_Y, SCORE_SHEET_HEIGHT } = CONSTANTS;

  doc.rect(10, SCORE_SHEET_START_Y, 126, SCORE_SHEET_HEIGHT);
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  doc.text("First Term (100)", 61, 99, null, 90);
  doc.text("Second Term (100)", 69, 99, null, 90);
  doc.text("Third Term (100)", 79, 99, null, 90);
  doc.text("Total (300)", 88, 99, null, 90);
  doc.text("Subject Avg.", 97, 99, null, 90);
  doc.text("Subject Highest", 105, 99, null, 90);
  doc.text("Subject Lowest", 114, 99, null, 90);
  doc.text("Subject Position", 123, 99, null, 90);
  doc.text("Subject Grade", 133, 99, null, 90);
...
function drawAnnualAffirmativeSkills(doc, affirmativeSkills) {
...
function drawAnnualPsychomotorSkills(doc, psychomotorSkills) {
...
function drawAnnualSummary(doc, summary) {
